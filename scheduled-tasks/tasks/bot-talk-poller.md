...
2. **If new messages found (from either sender):**
   ...
   - Format each message with a directional prefix:
     ...
     - AlbertLobster messages: `📥 AlbertLobster → owner: <content>`
   ...
   - Notify the owner via Telegram (chat_id=OWNER_CHAT_ID) with the full conversation block
     showing both sides.
...
📥 AlbertLobster → owner: I reviewed it. Looks reasonable, a few questions though.
📥 AlbertLobster → owner: Can you clarify item 3?
...
Only send a Telegram message to the owner (chat_id=OWNER_CHAT_ID) when there is genuinely new content:
